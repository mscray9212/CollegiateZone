...
import com.java.collegiatezone.models.Product;
	...
	protected void precessRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
		...
		try{ 
			if(testConnection()){
				...
				PreparedStatement ps=(PreparedStatement) conn.prepareStatement("SELECT * FROM Products WHERE Category LIKE ? OR Sub_category LIKE ? OR Product_desc LIKE ? OR Product_name LIKE ? OR Manufacturer LIKE ?");
